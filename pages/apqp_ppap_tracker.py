...
with tab_deepdive:
    ...
    st.markdown("- **Why:** This transforms the tool into an active workspace for the SQE. It demonstrates a deep, practical understanding of the PPAP process beyond high-level tracking. The references to standards like **IATF 16949** show broad quality systems knowledge.")
    ppap_elements = [{"Element": "Design Records", "Reference": "AS9145 / IATF", "Status": "Approved"}, {"Element": "Engineering Change Documents", "Reference": "AS9145 / IATF", "Status": "Approved"}, {"Element": "Process Flow Diagram", "Reference": "AS9145 / IATF", "Status": "Submitted"}, {"Element": "Process FMEA (PFMEA)", "Reference": "AS9145 / IATF", "Status": "Submitted"}, {"Element": "Control Plan", "Reference": "AS9145 / IATF", "Status": "Submitted"}, {"Element": "Measurement System Analysis (MSA)", "Reference": "AS9145 / IATF", "Status": "Rejected"}, {"Element": "Dimensional Results", "Reference": "AS9145 / IATF", "Status": "Approved"}, {"Element": "Initial Process Studies (Cpk)", "Reference": "AS9145 / IATF", "Status": "Submitted"}, {"Element": "Part Submission Warrant (PSW)", "Reference": "AS9145 / IATF", "Status": "Not Submitted"}]
    ...
    with col1:
        ...
        st.markdown("- **Why (Actionability):** A high **Risk Priority Number (RPN)** flags process steps that require robust mitigation in the Control Plan. An SQE must review this to ensure the supplier has adequately addressed all high-risk failure modes before approving the process.")
        pfmea_data = {"Process Step": ["Wire Bonding", "Die Attach", "Encapsulation"], "Severity (S)": [9, 10, 6], "Occurrence (O)": [3, 2, 4], "Detection (D)": [5, 6, 3]}
        pfmea_df = pd.DataFrame(pfmea_data); pfmea_df["RPN"] = pfmea_df["S"] * pfmea_df["O"] * pfmea_df["D"]
        st.dataframe(pfmea_df.sort_values("RPN", ascending=False), use_container_width=True, column_config={"RPN": st.column_config.ProgressColumn("RPN", min_value=0, max_value=1000)})
    with col2:
        st.markdown("##### Measurement System Analysis (Gage R&R)")
        st.markdown("- **Why (Actionability):** An MSA answers: \"Is our measurement system trustworthy?\" A rejected MSA (as shown here, >9%) is a valid reason to **reject a PPAP** and demand the supplier fix their measurement process first.")
        ...
        gage_df = pd.DataFrame(gage_results); total_grr = gage_df[gage_df["Source of Variation"] == "Total Gage R&R"]["% Contribution"].iloc[0]
        fig_gage = go.Figure(go.Indicator(mode = "gauge+number", value = total_grr, title = {'text': "Total Gage R&R (% Contribution)"}, gauge = {'axis': {'range': [None, 30]}, 'bar': {'color': "darkblue"}, 'steps': [{'range': [0, 1], 'color': 'green'}, {'range': [1, 9], 'color': 'yellow'}, {'range': [9, 30], 'color': 'red'}], 'threshold': {'line': {'color': "black", 'width': 4}, 'thickness': 0.75, 'value': 9}}))
        st.plotly_chart(fig_gage, use_container_width=True, key="gage_r_and_r")
        if total_grr > 9: st.error(f"**MSA Rejected:** Total Gage R&R of {total_grr}% exceeds the >9% threshold for rejection. The measurement system is not acceptable.", icon="🚨")
        else: st.success("MSA Acceptable.")
