...
with tab_monitor:
    ...
    with col1:
        ...
        st.markdown("- **Why:** It answers the question: \"Is the process stable and predictable?\" Any point outside the red control limits indicates a 'special cause' variation that must be investigated immediately. This is fundamental to **AS9100 clause 8.5.1.3** (Production process verification).")
        np.random.seed(42); spc_data = np.random.normal(loc=10.0, scale=0.15, size=50); spc_data[30:35] += 0.6
        fig_spc = go.Figure(); fig_spc.add_trace(go.Scatter(y=spc_data, mode='lines+markers', name='Measurement'))
        fig_spc.add_hline(y=10.0, line=dict(dash="dash", color="green"), name="Target"); fig_spc.add_hline(y=10.5, line=dict(dash="dot", color="red"), name="UCL"); fig_spc.add_hline(y=9.5, line=dict(dash="dot", color="red"), name="LCL")
    ...
    with col2:
        ...
        st.markdown("- **Why (Actionability):** This is a core Six Sigma metric that answers: \"Is the process *capable* of meeting our requirements?\" An aerospace industry target is typically **Cpk > 1.33**. A value below this is a major risk and a data-driven reason to demand process improvement from the supplier, as per **AS9145 (APQP)** requirements.")
        usl, lsl = 10.6, 9.4; mu, sigma = 10.05, 0.18; process_data = np.random.normal(mu, sigma, 200)
        ...
        if cpk < 1.33: st.error(f"**Action Required:** Cpk of {cpk:.2f} is below the target of 1.33. A process improvement plan is required.", icon="🚨")
        else: st.success(f"**Process Capable:** Cpk of {cpk:.2f} meets the target of 1.33.", icon="✅")
with tab_predict:
    st.header("Predictive Analytics for Proactive Quality")
    st.subheader("Future Performance Forecast (Prophet)")
    st.markdown("- **Why:** This shifts the SQE role from reactive to proactive. The alert system warns of potential threshold breaches weeks in advance, allowing for preventive actions.")
    @st.cache_data
...
with tab_act:
    ...
    if st.button("Generate SCAR PowerPoint"):
        with st.spinner("Creating SCAR..."):
            ...
            st.download_button(label="Download SCAR (.pptx)", data=ppt_buffer, file_name=f"SCAR_{selected_supplier.replace(' ', '_')}_{pd.Timestamp.now().strftime('%Y%m%d')}.pptx", mime="application/vnd.openxmlformats-officedocument.presentationml.presentation")
