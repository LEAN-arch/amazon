...
# This data must be defined before the tabs so it's available to all of them.
@st.cache_data
def get_npi_data():
    return pd.DataFrame({
...
with tab_decision:
    ...
    - **How:** The SQE sets the relative importance (weight) for each category in the sidebar. The tool then calculates a final weighted score for each supplier, providing a data-driven ranking.
    ...
    st.dataframe(
        ...
        column_config={"Weighted_Score": st.column_config.ProgressColumn("Final Weighted Score", min_value=50, max_value=100, format="%.1f")}
    )
...
with tab_audit:
    ...
    with col1:
        ...
        fig_gauge = go.Figure(go.Indicator(
            mode="gauge+number", value=audit_progress,
