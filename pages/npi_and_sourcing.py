...
def get_sourcing_data():
    ...
    sourcing_data = {
        ...
        'Export Control Compliant (1=Yes, 0=No)': [1, 1, 1, 1],
        'Avg SCAR Closure (Days)': [25, 18, 45, 15],
        # --- Technical & Process Capability ---
        'Rad-Hard Process Maturity (1-5)': [4, 3, 2, 5],
        ...
        'Volume Ramp Readiness (1-5)': [3, 4, 2, 5],
...
with tab_decision:
    ...
    st.dataframe(final_df.sort_values("Weighted_Score", ascending=False), use_container_width=True,
        column_config={
            "QMS_Score": st.column_config.ProgressColumn("QMS Score", min_value=0, max_value=100), "Tech_Score": st.column_config.ProgressColumn("Tech Score", min_value=0, max_value=100),
            "Cost_Score": st.column_config.ProgressColumn("Cost Score", min_value=0, max_value=100), "Scale_Score": st.column_config.ProgressColumn("Scale Score", min_value=0, max_value=100),
    ...
    with st.expander("**SME DEEP DIVE: How Scores Are Calculated (with Standard & Project Relevance)**", expanded=True):
        ...
        st.markdown("- **`AS9100D Certified`**: **Metric:** A binary check for certification to the Aerospace QMS standard. **Kuiper Relevance:** A **non-negotiable baseline** for any supplier providing mission-critical hardware. **Standards:** **AS9100D**, **ISO 9001**.\n- **`Export Control Compliant`**: **Metric:** A binary check confirming the supplier's systems are compliant with US export regulations. **Kuiper Relevance:** A **legal and program-critical showstopper**. Satellite technology is subject to strict **ITAR/EAR** regulations. **Standards:** **ITAR**, **EAR**.\n- **`Avg SCAR Closure (Days)`**: **Metric:** The average time taken to close a formal Corrective Action Request. **Kuiper Relevance:** Measures responsiveness and problem-solving velocity, which is critical to prevent cascading production line stoppages. **Standards:** Effectiveness of the corrective action process is a key requirement of **AS9100D Clause 10.2**.")
        st.markdown("---")
        st.markdown("#### Technical & Process Capability")
        st.markdown("- **`Rad-Hard Process Maturity`**: **Metric:** A 1-5 rating of the supplier's experience and controls for creating radiation-hardened devices. **Kuiper Relevance:** The most critical technical differentiator. ASICs in Low Earth Orbit (LEO) are constantly bombarded by radiation. **Standards:** **JEDEC JESD236**, **MIL-STD-883 Test Method 1019**.\n- **`Avg Cpk (Critical Parameters)`**: **Metric:** The average Process Capability Index for key parameters. **Kuiper Relevance:** Statistical proof of reliability. A high Cpk (>1.33) means the process will consistently produce parts well within specification. **Standards:** A core tool of **Six Sigma**, and a key deliverable for process validation under **AS9145 (APQP)**.\n- **`First Pass Yield (%)`**: **Metric:** The percentage of units that pass all tests on their first attempt without any rework. **Kuiper Relevance:** A primary indicator of process stability. Low FPY at Kuiper's scale translates to massive scrap costs. **Standards:** A Key Performance Indicator (KPI) used to monitor the effectiveness of a QMS under **ISO 9001/AS9100**.")
        st.markdown("---")
        st.markdown("#### Cost & Commercial")
        st.markdown("- **`Quoted Unit Cost ($)`**: **Metric:** The price per ASIC unit. **Kuiper Relevance:** Important for the commercial viability of a large-scale constellation.\n- **`Est. COPQ (% of Spend)`**: **Metric:** The estimated Cost of Poor Quality (scrap, rework, engineering support). **Kuiper Relevance:** This demonstrates a \"total cost of ownership\" mindset. A supplier with a low unit cost but high COPQ is a false economy. **Standards:** A foundational concept in **Six Sigma**.")
        st.markdown("---")
        st.markdown("#### Scalability & Supply Chain")
        st.markdown("- **`Volume Ramp Readiness`**: **Metric:** A 1-5 rating of the supplier's audited ability to scale to high volumes. **Kuiper Relevance:** Directly addresses the central challenge of \"aerospace quality at **unprecedented scale**.\" **Standards:** Relates to **AS9100D Clause 8.1 (Operational Planning)**.\n- **`Capacity Utilization (%)`**: **Metric:** The percentage of the supplier's total manufacturing capacity already in use. **Kuiper Relevance:** A supplier at 95% utilization is a major risk; they have no buffer for demand upside. **Standards:** A key input into the risk assessment for **AS9100D Clause 8.4**.\n- **`BCP Audit Score`**: **Metric:** A 1-5 rating of the supplier's Business Continuity Plan. **Kuiper Relevance:** Assesses supply chain resilience against disruptions. For a program with a tight launch cadence, supplier downtime is not an option. **Standards:** Relates to risk-based thinking from **ISO 31000**.")
