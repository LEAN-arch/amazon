...
with tab_decision:
    ...
    @st.cache_data
    def get_npi_data():
        return pd.DataFrame({'Supplier': ['Future Foundries LLC', 'Global Test Solutions', 'NextGen Packaging', 'AeroChip Test'], 'Part_Type': ['RF ASIC', 'Power Mgmt ASIC', 'Memory Controller', 'Power Mgmt ASIC'], 'QMS_Score': [85, 92, 78, 95], 'Tech_Score': [90, 85, 88, 98], 'Cost_Score': [70, 85, 90, 65], 'Scale_Score': [80, 82, 75, 95]})
    npi_df = get_npi_data().copy()
...
with tab_audit:
    ...
    selected_supplier_audit = st.selectbox("Select Supplier for Audit Review", npi_df['Supplier'].unique(), key="audit_supplier_select")
